...
export default function cloneFrame(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		label: "Open panel",
		description:
			"Open one of CLONE FRAME's panels (tabs) in the running app window and focus it. Keys: terminal (CODE), shell (iT terminal), research (web browser), harness, lab, matrix, machine, agents, folders, email, notes, tasks, calendar, reminders, contacts, library, search, brain, cookbook, compare, gallery, integrations, theme, settings. Titles like 'browser' (→research) or 'My Agents' (→agents) also resolve. This physically opens the tab on screen.",
	...
	pi.registerTool({
		...
		promptGuidelines: [
			"Use app_rpc for app data/actions that aren't a panel — searching the web via the app, reading files, listing/creating harnesses, reading notes/tasks, operating servers.",
