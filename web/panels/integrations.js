  function wireIntegrations(p){
    ...
    async function load(){let items=[];try{items=await RPC('integrations','list')}catch(e){showErr(body,e);return}
      body.innerHTML=items.length?items.map(g=>`<div class="igrow"><span class="igdot ${escAttr(g.status)}"></span><div><b>${escAttr(g.name)}</b> <span class="badge">${escAttr(g.type)}</span>${g.isDefault?' <span class="badge">default</span>':''}<div class="dim" style="font-size:9px">${metaStr(g)}${g.lastError?' · '+escAttr(g.lastError):''}</div></div><div style="margin-left:auto;display:flex;gap:6px">${g.readOnly?'':`<button class="btn mini" data-test="${g.id}">test</button><button class="btn mini" data-rm="${g.id}">✕</button>`}</div></div>`).join(''):'<div class="qempty">No integrations.</div>';
    ...
    p.querySelector('#igadd').addEventListener('click',()=>{
      ...
      function fields(){const v=t.value;
        if(v==='api')ff.innerHTML='<div class="af-row"><label>Base URL</label><input id="igu" placeholder="https://api.exemplo.com"></div><div class="af-row"><label>API key</label><input id="igk" type="password" placeholder="optional"></div>';
